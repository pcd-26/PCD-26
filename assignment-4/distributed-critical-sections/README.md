...
1. **Token Circulation Queue (`cs_token_circulation_<csName>`)**:
...
3. **Crash-Safe Bootstrap**:
   - Bootstrap is serialized with a temporary exclusive broker guard queue `cs_token_bootstrap_guard_<csName>`.
   - While holding that guard, a process passively inspects `cs_token_circulation_<csName>`.
...
```bash
mvn -f assignment-4/distributed-critical-sections/pom.xml exec:java -Dexec.args="Process-A localhost 5672"
```
