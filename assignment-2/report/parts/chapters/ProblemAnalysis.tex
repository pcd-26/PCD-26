...
The range \texttt{[0, MaxFS]} is divided into \texttt{NB} regular bands and an
additional overflow band stores files larger than \texttt{MaxFS}. Boundary
values are handled deterministically: a file exactly equal to \texttt{MaxFS}
belongs to the last regular band, while \texttt{MaxFS + 1} belongs to the
overflow band. This rule is centralized in \texttt{FSReport.getBandIndex()} and
is tested explicitly with boundary-sized files.
