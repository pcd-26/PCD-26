...
\texttt{FSStatLib} has to compute a report from three input parameters:
\textbf{\texttt{D}}, the root directory to scan; \textbf{\texttt{MaxFS}}, the
maximum file size used as threshold; and \textbf{\texttt{NB}}, the number of
regular size bands. The operation is simple as a sequential computation: visit
each file, read its size, update the total count, and increment the
corresponding histogram band. The concurrent aspect comes from doing this
\textbf{asynchronously} over a real filesystem, while still supporting
\textbf{progress updates} and \textbf{cancellation}.
...
The asynchronous API must return control to the caller without waiting for the
whole directory tree to be processed. At the same time, the implementation must
coordinate work that may still be in progress. In particular, it must define:
...
\textbf{callbacks and pending operations} in the event-loop implementation,
\textbf{stream accumulation} in the reactive implementation, and
\textbf{task coordination} in the virtual-thread implementation.
...
deterministically: a file exactly equal to \texttt{MaxFS} belongs to the
\textbf{last regular band}, while a file larger than \texttt{MaxFS} belongs to
the \textbf{overflow band}.
...
The expected behaviour can be summarized through five properties:

\begin{itemize}[leftmargin=*]
  \item \textbf{Safety}: every counted file contributes to exactly one size
  band;
  \item \textbf{Completeness}: every reachable regular file is counted once;
