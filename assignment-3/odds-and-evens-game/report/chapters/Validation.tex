...
\begin{itemize}
    ...
    \item invalid parity functions and invalid players are reported as \textbf{errors} instead of producing undefined behavior.
\end{itemize}
...
If the parity function is `nil`, or if it returns an invalid parity, the error is propagated after all workers have been
accounted for.
This keeps the failure path well defined and prevents the coordinator from advancing with incomplete information.

The \textbf{test suite} covers player validation, parity parsing, single-match behavior, round execution, full championship
...
every result, that result ordering is stable, and that invalid parity functions do not deadlock the round.
