...
The worker logic calls the parity function exactly once, maps \texttt{Odd} to the first player and \texttt{Even} to the
second player, and returns a
\texttt{MatchResult} with the round number, match number, participants, winner, and resulting parity.
...
for matchIndex := 0; matchIndex < matchCount; matchIndex++ {
    ...
    go func(matchIndex, matchNumber int, firstPlayer, secondPlayer domain.Player) {
        result, err := match.PlayMatch(roundNumber, matchNumber, drawParity, firstPlayer, secondPlayer)
...
for len(currentPlayers) > 1 {
    winners, matches, err := round.PlayRound(roundNumber, currentPlayers, drawParity)
    if err != nil {
