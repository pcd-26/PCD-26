...
Each match receives exactly two players and one \textbf{toss function}, then sends one result message back to the round coordinator.
...
Each \textbf{match} stores its round number, match number, the two participants, the winner, and the tossed side.
...
This keeps the rendering code simple, makes the tests easy to write, and avoids recomputing tournament history from the
final champion alone.
