...
func PlayRound(roundNumber int, players []domain.Player, decideWinnerParity func() domain.Parity) ([]domain.Player, []domain.MatchResult, error) {
	// The round needs a parity function to resolve each match.
	if decideWinnerParity == nil {
		return nil, nil, fmt.Errorf("parity function must not be nil")
	}
	...
	for matchIndex := 0; matchIndex < matchCount; matchIndex++ {
		...
		go func(matchIndex, matchNumber int, firstPlayer, secondPlayer domain.Player) {
			...
			result, err := match.PlayMatch(roundNumber, matchNumber, decideWinnerParity, firstPlayer, secondPlayer)
