...
### 4.4 Build and runtime requirements

- The project target is Java 17.
- The code can be compiled with a newer JDK, such as Java 21, as long as the
  generated bytecode remains compatible.
- The benchmark and report workflow must use a runtime that can execute the
  compiled classes, so Java 8 is not acceptable.
- If a benchmark run fails with `UnsupportedClassVersionError`, the first thing
  to check is whether `java` and `javac` point to different installations.
- The report should always mention the Java version used for the final benchmark
  snapshot, because reproducibility depends on it.
