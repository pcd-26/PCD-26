...
\begin{tikzpicture}[node distance=16mm and 16mm]
  ...
  \node[transition, right=of holephase] (dispatchc1) {Build\\Local Grid};
  \node[place, right=of dispatchc1] (localready) {Local\\Collision\\Work\\Ready};
  \node[transition, right=of localready] (joinc1) {Merge\\Grids};
  \node[place, right=of joinc1] (localdone) {Ordered\\Cells\\Ready};
  \node[transition, right=of localdone] (dispatchc2) {Resolve\\Owned Cells};
  \node[place, right=of dispatchc2] (crossready) {Sparse\\Deltas\\Ready};
  \node[transition, right=of crossready] (joinc2) {Merge\\Deltas};
  \node[place, right=of joinc2] (crossdone) {Commit\\Ready};
  ...
  \node[note, below=8mm of final, xshift=28mm] (semantics) {Abstract meaning: one controller-owned tick, worker-private parallel chunks, ordered cell resolution, and serialized commit};
