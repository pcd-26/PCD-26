...
changes. That mode runs the headless speedup benchmark plus the scalability
benchmark data needed for the sequential speedup charts, clears `results/`
and `charts/`, and then writes only the compact speedup chart set instead of
the full suite of charts.
