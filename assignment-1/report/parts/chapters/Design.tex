...
Each parallel phase uses $R$ workers or tasks. The diagram shows only $W_1$ and
$W_R$ for readability; the omitted workers follow the same private path. Their
blue transitions can fire in any order or concurrently. Private input places
ensure that each worker fires exactly once, while the join requires every
completion token. The net is therefore 1-safe and prevents the next phase from
starting early. Final delta application uses one sequential path for small sets
and multiple workers for large touched-ball sets.
