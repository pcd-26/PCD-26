...
stores the returned futures. The coordinator submits every task before waiting
on the futures, so \texttt{Future.get()} acts as a phase barrier rather than
serializing the range execution.
...
The uniform grid acts as the broad phase. Each worker builds private data and
collects candidate collision pairs; the coordinator then performs the ordered
merge and the sequential accumulation of collision contributions. The final
apply phase may update the actual board state in parallel, but only over
disjoint touched-ball ranges. This avoids conflicting writes while keeping the
physics result aligned with the sequential semantics.

Shutdown follows resource ownership. A runner stops ticks and closes the
mailbox. The threaded version joins controller and bot threads and verifies
that they terminated before closing its physics workers. The task-based version
waits for controller and bot executor termination before finally shutting down
the physics pool.
