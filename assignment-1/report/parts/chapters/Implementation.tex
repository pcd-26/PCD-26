...
The code is split into the \textbf{model}, the \textbf{physics engines}, the \textbf{runtime helpers}, and the \textbf{Swing view}, so the responsibilities stay visible in the source tree.
...
\begin{itemize}
    ...
    \item \texttt{CommandMailbox} and \texttt{GameLoop} (\path{pcd.poool.runtime}): command receipts, single-writer ticks, and snapshot coordination.
\end{itemize}
...
The shared \texttt{GameLoop} publishes snapshots through its monitor-protected latest-state field. External threads submit operations through \texttt{CommandMailbox}; the controller drains them \textbf{sequentially} and the mailbox completes the associated receipts.
