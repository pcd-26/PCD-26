...
The actual JPF run was executed through the Docker helper. The launcher first
compiles the reduced Java 11 harnesses and then checks both models:
\begin{verbatim}
python assignment-1/verification/jpf/run_jpf.py --model both
\end{verbatim}
The helper selects Docker when the host JVM is unsuitable, which keeps the
command portable across Windows and Linux. Both runs completed with
\texttt{no errors detected}.
