...
/**
 * Second-generation platform-threaded physics engine focused on reducing the
 * global coordination cost of the collision phase.
 *
 * <p>Compared to the first threaded engine, V2 keeps the movement phase
 * chunk-based but replaces the collision pipeline with a spatial-ownership
 * approach:
 *
 * <ol>
 ...
 * </ol>
 */
public class ThreadedPhysicsEngineV2 implements PhysicsStepper, AutoCloseable {
    ...
    /**
     * Creates a V2 engine using a CPU-oriented default worker count.
     */
    public ThreadedPhysicsEngineV2() {
     ...
     */
    public ThreadedPhysicsEngineV2(int workerCount) {
     ...
     */
    public ThreadedPhysicsEngineV2(int workerCount, long maxStepMillis) {
        ...
        for (int i = 0; i < workers.length; i++) {
            workers[i] = new PhysicsWorker("poool-physics-v2-worker-" + i);
        }
    }
    ...
    private void ensureOpen() {
        if (closed) {
            throw new IllegalStateException("threaded physics engine v2 is closed");
        }
    }
    ...
    /**
     * Immutable per-step profiling data for the V2 threaded physics pipeline.
     *
